...
digitze the channel of interest with as high dynamic range as possible while
minimzing the distortion by neighbouring channels. \\
...
the inherent frequency selectivity of the compontents is sufficient. \\

Since we are building an \gls{USBand} receiver, the \gls{RX} \gls{USBand} signal
at $+ f_{c} + f_{\text{RX LO}}$ is the desired signal.

The fourthe signal is the \gls{RX} \gls{LSBand} signal at
...
This filter is fixed in frequency even when channels at differenct carrier
...
\label{sec:rx_adc_0}
Instead of using two \gls{ADC} channels and qudrature sampling, the
...
correctly allined with a Nyquist-zone as shown in
...
we meet the specification of the \gls{ADC} and coverter boards. \\
